...
module.exports = {
    ...
    checkin: function() {
        ...
        this.client.get(this.options.registration_url, options, this.parse_eureka_reply.bind(this));
    },
    ...
    parse_eureka_reply: function(data, response) {
        ...
        if (response.statusCode == 200) {
            for (var service in this.instances) {
                delete(this.instances[service]);
            }
            if (!Array.isArray(data.applications.application)) {
                data.applications.application = [data.applications.application];
            }
            for (var service_index in data.applications.application) {
                // console.log("Processing " + data.applications.application[service_index].name);
